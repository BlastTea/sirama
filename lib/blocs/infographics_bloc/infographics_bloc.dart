...
class InfographicsBloc extends Bloc<InfographicsEvent, InfographicsState> {
  InfographicsBloc() : super(InfographicsInitial()) {
    ...
    on<InitializeInfographicsData>((event, emit) async {
      try {
        _infographicsList = await ApiHelper.get('/api/infografis').then((value) => (value.data['data'] as List).map((e) => Infografis.fromJson(e)).toList());
      } catch (e) {
  ...
  InfographicsDataLoaded get _infographicsDataLoaded => InfographicsDataLoaded(infografis: _infographicsList);
}
