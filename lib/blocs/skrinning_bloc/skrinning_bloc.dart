  ...
  SkrinningBloc() : super(SkrinningInitial()) {
    ...
    on<GetDetailSkrinning>((event, emit) async {
      try {
        ...
        Response response =  await ApiHelper.get('/api/detailskrinning/$idSkrinning');
        detailskrinning = (response.data['data'] as List).map((e) => DetailSkrinning.fromJson(e)).toList();
        skrinuser = (response.data['skrin_user'] as List).map((e) => SkrinUser.fromJson(e)).toList();
        emit(_skrinningDataLoaded);
      } catch (e) {
        emit(SkrinningError());
        return;
    ...
    on<SubmitJawabanSkrinning>((event, emit) async {
      try {
        ...
        final idSkrinUser = event.skrinuser;
